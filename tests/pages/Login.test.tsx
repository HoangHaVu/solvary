...
describe("Login", () => {
  ...
  it("zeigt einen Fehler bei fehlgeschlagenem Login", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("info@voltify.com"), {
  ...
  it("ruft login mit korrekten Daten auf", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("info@voltify.com"), {
      target: { value: "installateur@test.de" },
    });
    fireEvent.change(screen.getByPlaceholderText("Enter your password"), {
  ...
  it("zeigt weder Passwort-vergessen noch Angemeldet-bleiben", () => {
    render(<Login />);
  ...
  it("füllt Test-Account-Daten beim Klick auf Schnell-Login", () => {
    ...
    fireEvent.click(
      screen.getByText("installateur@test.de").closest("button")!,
    );
