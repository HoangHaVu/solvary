...
export function ProjectCard({ project, done }: ProjectCardProps) {
  ...
  const name =
    ...
    (project.lead
      ...
      : null) ??
    t("adminDashboard.projects.unknownCustomer");
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div
          className={`rounded-lg p-2 flex items-center gap-1.5 ${done ? "bg-green-500/10" : "bg-[#252525]"}`}
        >
        ...
        >
          ...
          <p className="text-xs font-bold text-white truncate">
            {project.investment != null
              ? `${project.investment.toLocaleString(i18n.language)} €`
              : "—"}
          </p>
        </div>
      </div>
      ...
      <p className="text-xs text-gray-500 text-right font-medium group-hover:text-brand-primary transition-colors">
        {t("adminDashboard.projects.details")}
      </p>
    </div>
  );
}
