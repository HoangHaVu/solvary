...
const EMPTY_CREATE = {
  title: "",
  type: "beratung" as Appointment["type"],
  date: "",
  startTime: "09:00",
  endTime: "10:00",
  location: "",
...
export default function CalendarPage() {
  ...
  const MONTH_NAMES = t("installerDashboard.calendar.monthNames", {
    returnObjects: true,
  }) as string[];
  const WEEKDAY_NAMES = t("installerDashboard.calendar.weekdayNames", {
    returnObjects: true,
  }) as string[];
  ...
  > = {
    beratung: {
      label: t("installerDashboard.calendar.types.beratung"),
      bg: "bg-blue-500/10",
    ...
    installation: {
      label: t("installerDashboard.calendar.types.installation"),
      bg: "bg-orange-500/10",
    ...
    abnahme: {
      label: t("installerDashboard.calendar.types.abnahme"),
      bg: "bg-green-500/10",
    ...
    partnermeeting: {
      label: t("installerDashboard.calendar.types.partnermeeting"),
      bg: "bg-purple-500/10",
  ...
  function formatDuration(start: string, end: string): string {
    ...
    if (diffMin < 60)
      return t("installerDashboard.calendar.durationMinutes", {
        minutes: diffMin,
      });
    const h = Math.floor(diffMin / 60);
    const m = diffMin % 60;
    return m === 0
      ? t("installerDashboard.calendar.durationHours", { hours: h })
      : t("installerDashboard.calendar.durationHoursMinutes", {
          hours: h,
          minutes: m,
        });
  }
  ...
  const [activePersonIds, setActivePersonIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const personColorMap = useMemo<Record<string, PersonColor>>(() => {
    ...
    teamProfiles.forEach((p, i) => {
      map[p.id] = PERSON_PALETTE[i % PERSON_PALETTE.length];
    });
    return map;
  }, [teamProfiles]);
  ...
  const displayedAppointments = isOwner
    ? teamAppointments.filter(
        (a) =>
          activePersonIds.size === 0 || activePersonIds.has(a.installer_id),
      )
    : appointments;
  ...
  async function handleCreate(ev: React.FormEvent) {
    ...
    await create({
      ...
      lead_id:
        createForm.customerMode === "lead" ? createForm.leadId || null : null,
      customer_name:
        createForm.customerMode === "lead"
          ? pickedLead
          ...
          : createForm.customerMode === "manual"
            ? createForm.customerName || null
            : null,
      customer_phone:
        createForm.customerMode === "lead"
          ? (pickedLead?.phone ?? null)
          : createForm.customerMode === "manual"
            ? createForm.customerPhone || null
            : null,
      customer_email:
        createForm.customerMode === "lead"
          ? (pickedLead?.email ?? null)
          : createForm.customerMode === "manual"
            ? createForm.customerEmail || null
            : null,
    });
    setIsSaving(false);
    setShowCreate(false);
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-8">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={goToday}
                className="px-5 py-2 bg-brand-secondary-hover border border-white/5 rounded-xl text-sm font-bold text-white hover:bg-white/5 transition-colors"
              >
                {t("installerDashboard.calendar.today")}
              </button>
              <button
              ...
              >
                ...
                {t("installerDashboard.calendar.newAppointment")}
              </button>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 mb-8">
            <div className="flex items-center gap-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest mr-2">
              ...
              {t("installerDashboard.calendar.legend")}
            </div>
            {(
              ["beratung", "installation", "abnahme"] as Appointment["type"][]
            ).map((t) => (
              <div
                key={t}
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full ${TYPE_CONFIG[t].bg} border ${TYPE_CONFIG[t].border} ${TYPE_CONFIG[t].text} text-xs font-bold`}
              >
                <span
                  className={`w-2 h-2 rounded-full ${TYPE_CONFIG[t].dot}`}
                />
                {TYPE_CONFIG[t].label}
              </div>
            ))}
        ...
        {selected &&
          (() => {
            ...
            const installerName = isOwner
              ? (teamProfiles.find((p) => p.id === selected.installer_id)
                  ?.full_name ?? null)
              : null;
            return (
              <div
              ...
              >
                <div
                  className="bg-brand-secondary-hover rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-white/10"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Modal-Header */}
                  <div
                    className={`${cfg.bg} ${cfg.border} border-b px-6 py-5 flex items-start justify-between gap-4`}
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`w-3 h-3 rounded-full shrink-0 mt-0.5 ${cfg.dot}`}
                      />
                      <div>
                        <p
                          className={`text-[10px] font-bold uppercase tracking-widest mb-0.5 ${cfg.text}`}
                        >
                          {cfg.label}
                        </p>
                    ...
                    <button
                      onClick={closeDetail}
                      className="shrink-0 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/10 text-gray-500 hover:text-white transition-colors"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                    ...
                    <>
                      <div className="px-6 py-5 space-y-4">
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-sm font-semibold text-white">
                            {formatFullDate(selected.starts_at)}
                          </p>
                        </div>
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="text-sm font-semibold text-white">
                              {t("installerDashboard.calendar.timeRange", {
                                start: formatTime(selected.starts_at),
                                end: formatTime(selected.ends_at),
                              })}
                            </p>
                        ...
                        <div className="flex items-start gap-3 pt-3 border-t border-white/5">
                          ...
                          {selected.customer_name ? (
                            ...
                            <p className="text-xs text-gray-500 italic mt-0.5">
                              Interner Termin — kein Kunde zugeordnet
                            </p>
                          )}
                        </div>
                        {installerName && (
                      ...
                      <div className="px-6 pb-5 flex gap-2">
                        ...
                        >
                          ...
                          {t("installerDashboard.calendar.reschedule")}
                        </button>
                        <button
                        ...
                        >
                          ...
                          {t("installerDashboard.calendar.cancel")}
                        </button>
                      </div>
                    </>
                  )}
                    ...
                    <form onSubmit={handleReschedule}>
                      <div className="px-6 py-5 space-y-4">
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                          {t("installerDashboard.calendar.rescheduleTitle")}
                        </p>
                        <div className="space-y-1.5">
                          <label className="text-xs font-semibold text-gray-400">
                            {t("installerDashboard.calendar.date")}
                          </label>
                          <input
                            required
                            type="date"
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-gray-400">
                              {t("installerDashboard.calendar.from")}
                            </label>
                            <input
                              required
                              type="time"
                          ...
                          <div className="space-y-1.5">
                            <label className="text-xs font-semibold text-gray-400">
                              {t("installerDashboard.calendar.to")}
                            </label>
                            <input
                              required
                              type="time"
                        ...
                        <div className="space-y-1.5">
                          <label className="text-xs font-semibold text-gray-400">
                            {t("installerDashboard.calendar.note")}
                          </label>
                          <textarea
                            rows={2}
                            ...
                            placeholder={t(
                              "installerDashboard.calendar.notePlaceholder",
                            )}
                      ...
                      <div className="px-6 pb-5 flex gap-2">
                        ...
                        >
                          ...
                          {isSaving
                            ? t("installerDashboard.calendar.saving")
                            : t("installerDashboard.calendar.reschedule")}
                        </button>
                        <button
                          type="button"
                        ...
                        >
                          {t("installerDashboard.calendar.back")}
                        </button>
                      </div>
                    </form>
                  )}
                    ...
                    <>
                      <div className="px-6 py-5">
                        <div className="bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-4 space-y-1">
                          <p className="text-sm font-bold text-red-400">
                            {t(
                              "installerDashboard.calendar.cancelConfirmTitle",
                            )}
                          </p>
                          <p className="text-sm text-red-400/80">
                            {t(
                              "installerDashboard.calendar.cancelConfirmText",
                              {
                                title: selected.title,
                                date: formatFullDate(selected.starts_at),
                              },
                            )}
                          </p>
                        </div>
                      ...
                      <div className="px-6 pb-5 flex gap-2">
                        ...
                        >
                          ...
                          {isSaving
                            ? t("installerDashboard.calendar.cancelling")
                            : t("installerDashboard.calendar.confirmCancel")}
                        </button>
                        <button
                          onClick={() => setModalMode("view")}
                          className="py-2.5 px-4 rounded-xl border border-white/10 text-sm font-bold text-gray-500 hover:bg-white/5 transition-colors"
                ...
                <div className="space-y-3 pt-1">
                    ...
                    {(["lead", "manual", "internal"] as const).map((mode) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setCreateForm((f) => ({ ...f, customerMode: mode }))
                          }
                  ...
                  {createForm.customerMode === "lead" && (
                    <select
                      ...
                      onChange={(e) =>
                        setCreateForm((f) => ({ ...f, leadId: e.target.value }))
                      }
                      className={inputCls}
                    >
    ...
    <div className="flex items-center gap-2">
      ...
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M19.5 4h-15A2.5 2.5 0 0 0 2 6.5v11A2.5 2.5 0 0 0 4.5 20h15a2.5 2.5 0 0 0 2.5-2.5v-11A2.5 2.5 0 0 0 19.5 4zm-15 1.5h15a1 1 0 0 1 1 1v11a1 1 0 0 1-1 1h-15a1 1 0 0 1-1-1v-11a1 1 0 0 1 1-1zM7 9h2v2H7V9zm4 0h2v2h-2V9zm4 0h2v2h-2V9zM7 13h2v2H7v-2zm4 0h2v2h-2v-2zm4 0h2v2h-2v-2z" />
        </svg>
        Google
      </a>
      <a
      ...
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M21 7.5V6c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-1.5l4 2V5.5l-4 2zM20 18H5V6h15v12z" />
        </svg>
        Outlook
      </a>
    </div>
